...
from search.models import ArticleType, ZhiHuQuestionType, LagouType, FoodType, MovieType
from django.http import HttpResponse
from datetime import datetime
from XiaoJianSouSuo.utils.common import OrderedSet
import redis
        ...
        if s_type == "article":
            ...
            for hit in response["hits"]["hits"]:
                ...
                try:
                    ...
                    hit_dict["source_site"] = "拉勾网/Boss直聘"
                    hit_list.append(hit_dict)
                except:
                    hit_dict["title"] = hit["_source"]["title"]
                    hit_dict["content"] = hit["_source"]["job_desc"]
            ...
            for hit in response["hits"]["hits"]:
                ...
                try:
                    ...
                    if "description" in hit["highlight"]:
                        hit_dict["content"] = "".join(hit["highlight"]["description"])
                    else:
                        hit_dict["content"] = hit["_source"]["description"]
                    hit_dict["url"] = hit["_source"]["url"]
                    hit_dict["create_date"] = hit["_source"]["create_date"]
                    hit_dict["score"] = hit["_score"]
                    hit_dict["advantage"] = hit["_source"]["advantage"]
                    hit_dict["source_site"] = "美食杰"
                    hit_list.append(hit_dict)
                except:
                    hit_dict["title"] = hit["_source"]["title"]
                    hit_dict["create_date"] = hit["_source"]["create_date"]
