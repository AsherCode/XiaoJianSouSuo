...
class ValidityTester(object):
    ...
    async def test_single_proxy(self, proxy):       # 异步检测py3.5以上
        """
        ...
        try:
            async with aiohttp.ClientSession() as session:
                try:
                    ...
                    print('Testing', proxy)
                    async with session.get(self.test_api, proxy=real_proxy, timeout=get_proxy_timeout) as response:  # 请求百度地址连接
...
class Schedule(object):
    ...
    def run(self):
        ...
        valid_process = Process(target=Schedule.valid_proxy)        # 定时检测
        check_process = Process(target=Schedule.check_pool)         # 从网上获取可用ip
        valid_process.start()                                       # 双进程同时运行
        check_process.start()
